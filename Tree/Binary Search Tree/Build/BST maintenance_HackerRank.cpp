...
int was[maxn], iteration, sn_sz, sub_nodes[maxn], subtree[maxn], q, f[maxn], t[maxn], p[maxn], n, i, anc, tn, ii, _cut_vertex, j, dst[maxn], queue[maxn], q1, q2, sp, st[maxn], Last[maxn], a[maxn], bst_left[maxn], bst_right[maxn];
    ...
    while (q1 != q2){
        ...
        while (q > 0) {
            if (was[t[q]] != iteration && !cut[t[q]]) {
                ...
                dst[t[q]] = cur_dist + 1;
                queue[q1++] = t[q];
                belongs[t[q]].push_back(info(_cut_vertex, cur_dist + 1, sub_idx));
            }
            q = p[q];
        }
    }
...
void add_vertex (int k) 
{
    for(j = 0; j < belongs[k].size(); j++) 
	{
        ...
        if (cur.cut_vertex != k) 
		{
            ...
            sum_of_subtree_distances[cur.cut_vertex][cur.subtree_idx] += cur.cur_dist;
            sum_of_distances[cur.cut_vertex] += cur.cur_dist;           
            ret += cur.cur_dist * (num_of_part_vertexes[cur.cut_vertex] - num_of_subtree_added[cur.cut_vertex][cur.subtree_idx]);
            ret += sum_of_distances[cur.cut_vertex] - sum_of_subtree_distances[cur.cut_vertex][cur.subtree_idx];
        }
		else 
